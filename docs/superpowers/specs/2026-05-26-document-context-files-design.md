...
1. **Focus the AI** — they tell the assistant *"these specific files are the context for
   this note,"* so it answers questions like *"what does question 3 mean?"* against the
...
- Contents: list of attached files (type icon + name + source label + **✕ remove**); an
  ...
  *"No files attached — that's fine. The AI still answers using everything in this course.
  Attach the exercise sheet or slides to give it focused context."*
...
3. Add a short context/system line naming them:
   *"The student attached these files as the primary context for this note: <names>. Treat
   them as the main reference and assume questions refer to them unless stated otherwise."*
