...
All in `extension/store-assets/`, at exact store-required dimensions. Upload
the four screenshots in numeric order and the promo tile in the marketing slot.

- [x] **Icon** — `extension/icons/icon-128.png` (128 × 128)
- [x] **01-extension-connected.png** (1280 × 800) — popup's "Connected Moodle
      sites" view with two institutions linked
- [x] **02-dashboard.png** (1280 × 800) — real Typenote dashboard: the course
      plus the imported documents grid (the payoff)
- [x] **03-course-materials.png** (1280 × 800) — real course view with the
      imported documents and the Import / Share actions
...
- **Extension popup shots + promo tile** (`01`, `04`, `promo-tile`) are
  reproducible — composited from the REAL popup (`popup.css` + the exact markup
  `popup.ts` renders) onto store-sized canvases. Regenerate from the repo root
  (Playwright lives in the root workspace, not in `extension/`):

  ```sh
  node extension/scripts/render-screenshots.mjs
  ```

- **Dashboard shots** (`02`, `03`) are live captures of the production app
  (`typenote-two.vercel.app`) signed in as the `beta-user-19@typenote.dev` test
  account. They are committed as static deliverables; the one-off capture
  script was intentionally not kept in the repo (it minted a prod session via
  the service-role key). To refresh them, re-shoot from a logged-in test
  account at 1280 × 800.
